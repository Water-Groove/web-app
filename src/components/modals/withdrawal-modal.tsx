...
import { InvestorBalancesDto } from "@/types/type";
...
export function WithdrawalModal({
  ...
  const validateAmount = () => {
    ...
    if (amount < MIN_WITHDRAWAL)
      return `Minimum withdrawal is ${formatAmount(MIN_WITHDRAWAL)}`;
    if (amount > maxWithdrawableAmount) {
      if (earlyWithdrawal) {
        return `Amount exceeds available balance of ${formatAmount(
          maxWithdrawableAmount
        )}`;
      } else {
        return `Maximum withdrawal without penalty is ${formatAmount(
          maxWithdrawableAmount
        )}`;
      }
    }
  ...
  const handleSubmit = async (data: WithdrawalFormValues) => {
    ...
    formData.append("investmentId", data.investmentId.toString());
                    ...
                    {principalLocked > 0 && (
                      <div className="text-xs">
                        ...
                        <span className="font-medium text-amber-600">
                          {formatCurrency(principalLocked)}
                        </span>
                      </div>
                    )}
                    {interestEarned > 0 && (
                  ...
                  {earlyWithdrawal && principalBeingWithdrawn > 0 && (
                    <>
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {formatAmount(principalBeingWithdrawn)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-wg-primary/70">
            ...
            <Form {...form}>
              ...
              >
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-3 p-3 rounded-lg border border-amber-200 hover:bg-amber-50 transition-colors">
                              ...
                              >
                                ...
                                <div className="text-sm text-wg-primary/70">
                                  Withdraw principal + interest (25% penalty on
                                  principal)
                                </div>
                              </Label>
                            </div>
                    ...
                    <AlertDescription className="text-amber-800 text-sm">
                      ...
                      {principalBeingWithdrawn > 0 && (
                        <span className="block mt-1">
                          Penalty on {formatAmount(principalBeingWithdrawn)}{" "}
                          principal:{" "}
                    ...
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent className="bg-white border-wg-primary/20">
                          {investmentsBalance.map((investment) => (
                            <SelectItem
                              key={investment.id}
                            ...
                            >
                              <div className="flex flex-row">
                                <span className="font-medium text-wg-primary">
                                  PL-{formatCurrency(investment.principalLocked)}
                                </span>{", "}
                                <span className="font-medium text-wg-primary">
                                  {" "}AVB-{formatCurrency(investment.availableBalance)}
                                </span>
                              </div>
                            </SelectItem>
                          ))}
                ...
                <div className="space-y-4">
                  <h3 className="text-wg-primary font-medium">
                    Bank Account Details
                  </h3>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-wg-primary">
                          Account Holder Name
                        </FormLabel>
                        <FormControl>
                          <Input
                ...
                {amount > 0 && (
                  <div className="p-4 bg-gradient-to-r from-wg-secondary/5 to-wg-primary/5 rounded-lg border border-wg-primary/20">
                    <h4 className="font-medium text-wg-primary mb-2">
                      Withdrawal Summary
                    </h4>
                ...
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:pt-4">
                  ...
                  <Button
                    type="submit"
                    disabled={
                      isPending || !!amountError || amount < MIN_WITHDRAWAL
                    }
